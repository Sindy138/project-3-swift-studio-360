...
const Dashboard = () => {
  ...
  const activeCount = orders.filter((o) => o.status === 'PENDING' || o.status === 'PROGRESS').length
  const doneCount = orders.filter((o) => o.status === 'DONE').length

  if (loading) {
    return <main className={styles.page}><div className={styles.loading}>Cargando tu dashboard...</div></main>
  }
    ...
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        <div className={styles.header}>
          ...
          <Link to="/perfil" className={styles.profileLink}>Mi perfil →</Link>
        </div>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHeader}>
            ...
            <Link to="/servicios" className={styles.newOrderBtn}>+ Nuevo servicio</Link>
          </div>

          {orders.length === 0 ? (
            <div className={styles.empty}>
              <p>Todavía no tienes ningún pedido.</p>
              <Link to="/servicios" className={styles.emptyLink}>Explorar servicios →</Link>
            </div>
          ) : (
                ...
                >
                  <div className={styles.orderInfo}>
                    ...
                    <p className={styles.orderMeta}>
                      {order.service?.category} · {new Date(order.createdAt).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                    </p>
                  </div>
                  <div className={styles.orderRight}>
                    <span className={styles.orderPrice}>{order.total.toLocaleString('es-ES')} €</span>
